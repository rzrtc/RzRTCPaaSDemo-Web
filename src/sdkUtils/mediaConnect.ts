...
import { RzRTC, getClient } from '../rz-paas-sdk'
...
const createLocalTracks = (isCreateVideoTrack: boolean = true, isCreateAudioTrack: boolean = true): Promise<void> => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (isCreateAudioTrack === true && (store.settingsStore.micId !== DeviceID.close)) {
      try {
        const track = await RzRTC.createMicrophoneAudioTrack({ microphoneId: store.settingsStore.micId })
        await client.publish(track)
...
const initTracks = () => {
  ...
  if (store.settingsStore.cameraId === DeviceID.empty) {
    getDeviceRes.push(RzRTC.getCameras(true).then((devices) => {
      if (devices.length !== 0) {
  ...
  if (store.settingsStore.micId === DeviceID.empty) {
    getDeviceRes.push(RzRTC.getMicrophones(true).then((devices) => {
      if (devices.length !== 0) {
